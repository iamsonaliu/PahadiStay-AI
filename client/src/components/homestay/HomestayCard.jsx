...
function Stars({ rating }) {
  return (
    <span className="flex items-center gap-1 text-sm">
      ...
      <span className="font-semibold text-gray-700 dark:text-cream-100">{rating}</span>
    </span>
  )
}
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-end justify-between border-t border-cream-200 dark:border-white/10 pt-3">
          <div>
            <span className="font-bold text-forest-700 dark:text-forest-200 text-xl">
              ₹{pricePerNight?.toLocaleString('en-IN')}
            </span>
            <span className="text-gray-400 text-sm"> / night</span>
          </div>
          <span className="text-xs text-gray-400">
            {totalReviews > 0 ? `${totalReviews} review${totalReviews !== 1 ? 's' : ''}` : 'New'}
          </span>
        </div>
      </div>
    </Link>
